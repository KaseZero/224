...
class common(object):
    ...
    def local_eles(self,type,value):
        el = None
        if type == "id":
            try:
                el = self.dr.find_elements(By.ID,value)
            except:
                self.dr.get_screenshot_as_file('../img/元素类型'+type+'-值'+value+'未定位到.jpg')
        elif type == "name":
            try:
                 el = self.dr.find_elements(By.NAME,value)
            except:
                self.dr.get_screenshot_as_file('../img/元素类型'+type+'-值'+value+'未定位到.jpg')
        elif type == "xpath":
            try:
                 el = self.dr.find_elements(By.XPATH,value)
            except:
                self.dr.get_screenshot_as_file('../img/元素类型'+type+'-值'+value+'未定位到.jpg')

        elif type == "nclass":
            try:
                el = self.dr.find_elements(By.CLASS_NAME,value)
            except:
                self.dr.get_screenshot_as_file('../img/元素类型' + type + '-值' + value + '未定位到.jpg')
        elif type == "tag":
            try:
                el = self.dr.find_elements(By.TAG_NAME,value)
            except:
                self.dr.get_screenshot_as_file('../img/元素类型' + type + '-值' + value + '未定位到.jpg')
        elif type =='css':
            try:
                el = self.dr.find_elements(By.CSS_SELECTOR,value)
            except:
                self.dr.get_screenshot_as_file('../img/元素类型' + type + '-值' + value + '未定位到.jpg')
        elif type == "linkt":
            try:
                el = self.dr.find_elements(By.LINK_TEXT,value)
            except:
                self.dr.get_screenshot_as_file('../img/元素类型' + type + '-值' + value + '未定位到.jpg')
        elif type == "linkp":
            try:
                el = self.dr.find_elements(By.PARTIAL_LINK_TEXT,value)
            except:
                self.dr.get_screenshot_as_file('../img/元素类型' + type + '-值' + value + '未定位到.jpg')
        return el
    ...
    def get_at(self,type,value,at):
        ...
        return a
    def frame(self,type,value):
        st = self.local_ele(type,value)
        self.dr.switch_to.frame(st)
        # self.dr.switch_to.iframe(st)
    #下拉列表的操作
